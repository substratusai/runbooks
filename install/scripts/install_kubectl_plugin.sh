...
LATEST_UPLOAD_KIND_ARTIFACT_URL=$(echo $LATEST_RELEASE | awk -v repo=$REPO -v os=$OS -v arch=$ARCH -v release=$LATEST_RELEASE '{print repo "/releases/download/" release "/kubectl-upload-resource_" os "_" arch ".tar.gz"}')

wget -qO- ${LATEST_OPEN_NOTEBOOK_ARTIFACT_URL} | tar zxv
wget -qO- ${LATEST_UPLOAD_KIND_ARTIFACT_URL} | tar zxv
